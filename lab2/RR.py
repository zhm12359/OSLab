...
while len(finished) != total:
    ...
    new_ready.extend([x for x in process_list if x.A == cycle and x not in ready])
    ...
    if not running:
        ...
        if current.remainingTime == 0:
            ...
            new_ready = [n for n in process_list if n in new_ready]
            ready = [r for r in ready if r not in new_ready]
